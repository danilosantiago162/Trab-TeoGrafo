...
int diametro(Grafo *g) {
    int diametro_atual = 0;

    // Verificação de grafo vazio
    if (g->n <= 1) {
        return 0;
    }
    ...
    for (int u = 0; u < g->n; u++) {
        // Obtenha as distâncias de 'u' para todos os outros vértices
        int *distancias = bfs_distancias(g, u + 1);

        // Encontre a maior distância a partir de 'u'
        int max_dist_u = 0;
        for (int v = 0; v < g->n; v++) {
            if (distancias[v] > max_dist_u) {
                max_dist_u = distancias[v];
            }
        }

        // Se a maior distância de 'u' for maior que o diâmetro atual,
        // atualize o diâmetro
        if (max_dist_u > diametro_atual) {
            diametro_atual = max_dist_u;
        }

        // Libere a memória alocada para as distâncias
        free(distancias);
    }
    return diametro_atual;
}

// Função auxiliar DFS para contar vértices em uma componente
...
// Encontra e salva as componentes conexas do grafo
